...
def prepare_example(input_file_path, output_directory):
    ...
    with PdfPages(out_pdf_file_path) as pdf:
        ...
        c1_dists = np.dot(X[class_1], dec_vec)
        c2_dists = np.dot(X[class_2], dec_vec)

        ## distances histogram
        plt.hist(c1_dists,color='red',alpha=0.4, density=True, label="class 1")
        plt.hist(c2_dists, color='green', alpha=0.4, density=True, label="class 2")
